    ...
    exports.createGoal = async (req, res) => {
        const { userId, name, targetAmount, currentAmount = 0, startDate, targetDate, status = 'In Progress', type } = req.body;
        const id = uuidv4();

        if (!userId || !name || !targetAmount || !startDate || !type) {
            return res.status(400).json({ message: 'Missing required goal fields.' });
        }

        try {
            const sql = `INSERT INTO goals (id, userId, name, targetAmount, currentAmount, startDate, targetDate, status, type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`;
            const params = [id, userId, name, targetAmount, currentAmount, startDate, targetDate || null, status, type];
            await db.run(sql, params);
            res.status(201).json({ message: 'Goal created successfully!', goal: { id, userId, name, targetAmount, currentAmount, startDate, targetDate, status, type } });
        } catch (error) {
    ...
    exports.getGoalsByUserId = async (req, res) => {
        ...
        try {
            const goals = await db.all('SELECT * FROM goals WHERE userId = ? ORDER BY targetDate ASC', [userId]);
